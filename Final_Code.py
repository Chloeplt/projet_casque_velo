...
path="C:/CasqueVelo/test_12.mp4" #create a path to locate the file
cap = cv2.VideoCapture(path) #Call cv2.VideoCapture to display the video
#cx2.VideoCaptur(0) in order to test live by computer's camera
path1="C:/CasqueVelo/test_13.mp4" #create a path for the measuring the center of eyes
...
def center_eye():
    while True:
        ...
        for cnt in contours1:
            ...
            eye_cords=np.append(a, [xc+int(wc/2), yc+int(hc/2)])
            print(eye_cords)
            break
    return eye_cords
...
print("\nThe center of the pupil is at:", eye_center) #print the cordinates of the center
...
while True:
    ...
    for cnt in contours:
        ...
        #check if looking elsewhere function is true
